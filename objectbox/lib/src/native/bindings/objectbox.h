...
/// @return OBX_ERROR_ILLEGAL_STATE if the given transaction is not a write transaction.
OBX_C_API obx_err obx_txn_mark_success(OBX_txn* txn, bool wasSuccessful);
...
/// @param id non-zero
OBX_C_API obx_err obx_cursor_put(OBX_cursor* cursor, obx_id id, const uint8_t* data, size_t size);
...
OBX_C_API obx_err obx_cursor_put4(OBX_cursor* cursor, obx_id id, const uint8_t* data, size_t size, OBXPutMode mode);
...
/// @param id non-zero
OBX_C_API obx_err obx_cursor_put_new(OBX_cursor* cursor, obx_id id, const uint8_t* data, size_t size);
...
OBX_C_API obx_err obx_cursor_insert(OBX_cursor* cursor, obx_id id, const uint8_t* data, size_t size);
...
OBX_C_API obx_err obx_cursor_update(OBX_cursor* cursor, obx_id id, const uint8_t* data, size_t size);
...
OBX_C_API obx_err obx_cursor_get(OBX_cursor* cursor, obx_id id, const uint8_t** data, size_t* size);
...
OBX_C_API obx_err obx_cursor_first(OBX_cursor* cursor, const uint8_t** data, size_t* size);

OBX_C_API obx_err obx_cursor_next(OBX_cursor* cursor, const uint8_t** data, size_t* size);

OBX_C_API obx_err obx_cursor_seek(OBX_cursor* cursor, obx_id id);

OBX_C_API obx_err obx_cursor_current(OBX_cursor* cursor, const uint8_t** data, size_t* size);
...
OBX_C_API obx_err obx_box_put(OBX_box* box, obx_id id, const uint8_t* data, size_t size);

/// Convenience for obx_box_put5() with OBXPutMode_INSERT.
/// @param id non-zero
/// @returns OBX_ERROR_ID_ALREADY_EXISTS if an insert fails because of a colliding ID
OBX_C_API obx_err obx_box_insert(OBX_box* box, obx_id id, const uint8_t* data, size_t size);

/// Convenience for obx_cursor_put4() with OBXPutMode_UPDATE.
/// @param id non-zero
/// @returns OBX_ERROR_ID_NOT_FOUND  if an update fails because the given ID does not represent any object
OBX_C_API obx_err obx_box_update(OBX_box* box, obx_id id, const uint8_t* data, size_t size);

/// Put the given object using the given ID synchronously; note that the ID also must match the one present in data.
/// @param id An ID usually reserved via obx_box_id_for_put().
/// @see obx_box_put() for standard put mode
/// @see obx_box_put_object() for a variant not requiring reserving IDs
OBX_C_API obx_err obx_box_put5(OBX_box* box, obx_id id, const uint8_t* data, size_t size, OBXPutMode mode);
...
OBX_C_API obx_err obx_async_put(OBX_async* async, obx_id id, const uint8_t* data, size_t size);

/// Put asynchronously using the given mode.
OBX_C_API obx_err obx_async_put5(OBX_async* async, obx_id id, const uint8_t* data, size_t size, OBXPutMode mode);

/// Put asynchronously with inserts semantics (won't put if object already exists).
OBX_C_API obx_err obx_async_insert(OBX_async* async, obx_id id, const uint8_t* data, size_t size);

/// Put asynchronously with update semantics (won't put if object is not yet present).
OBX_C_API obx_err obx_async_update(OBX_async* async, obx_id id, const uint8_t* data, size_t size);
...
OBX_C_API obx_qb_cond obx_qb_equals_bytes(OBX_query_builder* builder, obx_schema_id property_id, const uint8_t* value,
                                          size_t size);
...
OBX_C_API obx_qb_cond obx_qb_less_than_bytes(OBX_query_builder* builder, obx_schema_id property_id, const uint8_t* value,
                                             size_t size);
...
OBX_C_API obx_err obx_query_offset(OBX_query* query, uint64_t offset);
...
/// without limit.
OBX_C_API obx_err obx_query_offset_limit(OBX_query* query, uint64_t offset, uint64_t limit);
...
OBX_C_API obx_err obx_query_limit(OBX_query* query, uint64_t limit);
...
OBX_C_API obx_err obx_query_find_first(OBX_query* query, const uint8_t** data, size_t* size);
...
OBX_C_API obx_err obx_query_param_alias_bytes(OBX_query* query, const char* alias, const uint8_t* value, size_t size);
...
OBX_C_API obx_err obx_bytes_array_set(OBX_bytes_array* array, size_t index, const uint8_t* data, size_t size);
